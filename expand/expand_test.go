...
func TestBackslashInDoublequote(t *testing.T) {
	tests := []struct {
		test string
		src  string
		want string
		...
		{
			test: `Backslash for special character (\) preserves special character`,
			src:  `Hello \\ World`,
			want: `Hello \ World`,
		},
		{
		...
		{
			test: "Backslash for non-special character (b) is preserved",
			src:  `Hello \b World`,
			want: `Hello \b World`,
		},
		{
			test: "No backkslash is no-op",
			src:  `Hello World`,
			want: `Hello World`,
		},
		{
			test: `Multiple special characters are be preserved`,
			src:  `Hello \"\"\" World`,
			want: `Hello """ World`,
		},
		{
			test: `Multiple backslashes are preserved`,
			src:  `Hello \\\\ World`,
			want: `Hello \\ World`,
		},
	}
	...
	for _, tc := range tests {
		t.Run(tc.test, func(t *testing.T) {
			got := cfg.handleDoubleQuotedEscapeCharacters(tc.src)
			if got != tc.want {

...
func TestKillNonPositivePID(t *testing.T) {
	needsSubprocesses(t)
	if runtime.GOOS == "windows" || runtime.GOOS == "plan9" {
		t.Skip("no signals to send on this platform")
	}
	t.Parallel()
	// kill(2) reads 0 as the caller's process group and -1 as every process
	// the user may signal. An embedded interpreter must not let a script
	// reach the host that way, so both are refused and the shell runs on.
	for _, spec := range []string{"0", "-- -1"} {
		s := runSrc(t, "kill "+spec+"; echo st=$?; echo survived")
		if !strings.Contains(s, "st=1") || !strings.Contains(s, "survived") {

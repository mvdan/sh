...
set +o pipefail
 #IGNORE`,
	...
	*) echo "invalid option $REPLY"; break;;
	...
	{`set -- x y z; IFS=-; a=$*; echo "$a"`, "x y z\n #IGNORE TODO: bash prints x-y-z"},
	{`set -- x y z; IFS=; a=$*; echo "$a"`, "x y z\n #IGNORE TODO: bash prints xyz"},
	{`a=(x y z); IFS=; echo ${a[*]}; c=${a[*]}; echo "$c"`, "x y z\nx y z\n #IGNORE TODO: bash prints x y z, xyz"},
	{`a=(x y z); IFS=-; b=${a[*]}; echo "$b"`, "x y z\n #IGNORE TODO: bash prints x-y-z"},
	{`set -- x y; IFS=éz; a=$*; echo "$a"`, "x y\n #IGNORE TODO: bash prints xéy"},
	{`set -- xo yo; IFS=-; a=${*%o}; echo "$a"`, "x y\n #IGNORE TODO: bash prints x-y"},
	{`a=(zo wo); IFS=-; b=${a[*]^}; echo "$b"`, "Zo Wo\n #IGNORE TODO: bash prints Zo-Wo"},
	{`a=(x y z); IFS=-; echo "${!a[*]}"`, "0 1 2\n #IGNORE TODO: bash prints 0-1-2"},
	{`INTERP_Y_1=a INTERP_Y_2=b; IFS=-; echo "${!INTERP_Y_*}"`, "INTERP_Y_1 INTERP_Y_2\n #IGNORE TODO: bash joins with -"},
